# Configuration file for the Sphinx documentation builder.
#
# For the full list of built-in configuration values, see the documentation:
# https://www.sphinx-doc.org/en/master/usage/configuration.html

# -- Project information -----------------------------------------------------
# https://www.sphinx-doc.org/en/master/usage/configuration.html#project-information

project = 'Kamyar Hasanzadeh'
copyright = 'Kamyar Hasanzadeh'
author = 'Kamyar Hasanzadeh'
release = '2025'

kh_profile = {
    "name": "Dr. Kamyar Hasanzadeh",
    "lines": [
        "Lecturer · Adjunct Professor",
        "Geoinformatics & Urban Informatics",
        "University of Helsinki",
    ],
    "details": [
        {"icon": "map", "text": "Helsinki, Finland"},
        {"icon": "university", "text": "University of Helsinki"},
        {"icon": "link", "text": "Website", "href": "https://researchportal.helsinki.fi/en/persons/kamyar-hasanzadeh/"},
        {"icon": "home", "text": "MapSyntax", "href": "https://mapsyntaxhub.com/"},
        {"icon": "mail", "text": "Email", "href": "mailto:kamyar.hasanzadeh[at]helsinki.fi"},
        {"icon": "github", "text": "Github", "href": "https://github.com/kamyar68"},
        {"icon": "scholar", "text": "Google Scholar", "href": "https://scholar.google.fi/citations?user=XBp2BEEAAAAJ&hl=fi"},
        {"icon": "orcid", "text": "ORCID", "href": "https://orcid.org/0000-0002-0705-7662"},
        {"icon": "linkedin", "text": "LinkedIn", "href": "https://www.linkedin.com/in/kamyar-hasanzadeh-6257a227/"},

    ],
}


# -- General configuration ---------------------------------------------------
# https://www.sphinx-doc.org/en/master/usage/configuration.html#general-configuration

extensions = ['nbsphinx']

templates_path = ['_templates']
exclude_patterns = ['_build', 'Thumbs.db', '.DS_Store']



# -- Options for HTML output -------------------------------------------------
# https://www.sphinx-doc.org/en/master/usage/configuration.html#options-for-html-output

html_baseurl = "https://www.kamyarhz.com/"
html_last_updated_fmt = "%d %B %Y"
html_logo = "_static/logo/kamyarh.png"
html_short_title = "Kamyar H."
html_title = ""
html_css_files = [
    "https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,500;9..144,600&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap",
    "custom.css",
]
html_favicon = "_static/logo/favicon.png"
html_extra_path = ["_extra"]


html_context = {
    "kh_profile": kh_profile,
}



html_theme = "furo"

kh_palette_light = {
    "color-brand-primary": "#8a3b23",
    "color-brand-content": "#8a3b23",
    "color-background-primary": "#fdfbf7",
    "color-background-secondary": "#f5efe3",
    "color-background-hover": "#efe6d3",
    "color-foreground-primary": "#1c2330",
    "color-foreground-secondary": "#4f5b6b",
    "color-foreground-muted": "#7c8794",
    "color-foreground-border": "#e6ddc9",
    "color-background-border": "#e6ddc9",
    "font-stack": "'Inter', -apple-system, 'Segoe UI', sans-serif",
    "font-stack--headings": "'Fraunces', Georgia, 'Times New Roman', serif",
    "font-stack--monospace": "'JetBrains Mono', 'SFMono-Regular', Menlo, monospace",
}

kh_palette_dark = {
    "color-brand-primary": "#e6a27e",
    "color-brand-content": "#e6a27e",
    "color-background-primary": "#12151c",
    "color-background-secondary": "#181c25",
    "color-background-hover": "#212632",
    "color-foreground-primary": "#eef1f6",
    "color-foreground-secondary": "#aab3c2",
    "color-foreground-muted": "#7e8896",
    "color-foreground-border": "#2a2f3a",
    "color-background-border": "#2a2f3a",
}

html_theme_options = {
    "sidebar_hide_name": True,
    "light_css_variables": kh_palette_light,
    "dark_css_variables": kh_palette_dark,
    "source_repository": "https://github.com/MapSyntax/kamyar/",
    "source_branch": "main",
    "source_directory": "docs/",
    "footer_icons": [
        {
            "name": "GitHub",
            "url": "https://github.com/MapSyntax/kamyar",
            "html": (
                '<svg stroke="currentColor" fill="currentColor" stroke-width="0" '
                'viewBox="0 0 16 16"><path fill-rule="evenodd" d="M8 0C3.58 0 0 3.58 0 '
                '8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49'
                '-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01'
                '-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07'
                '-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 '
                '0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 '
                '2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 '
                '3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38'
                'A8.013 8.013 0 0 0 16 8c0-4.42-3.58-8-8-8z"></path></svg>'
            ),
            "class": "",
        },
    ],
}
html_static_path = ['_static']

nbsphinx_execute = "always"
nbsphinx_timeout = 120  # needed, e.g., when matplotlib updates its font cache
nbsphinx_allow_errors = True
